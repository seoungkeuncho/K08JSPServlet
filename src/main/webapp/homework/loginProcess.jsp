...
//application 내장객체를 통해 web.xml에 입력된 DB접속 정보를 읽어옴
...
MembershipDTO membershipDTO = dao.getMembershipDTO(userId, userPwd);
//DB작업이 끝나면 자원해제
dao.close();
...
if(membershipDTO.getId() !=null){
	...
	*/
	...
	기능이므로 포워드 된 페이지까지 데이터를 공유 할 수 있다.
	*/
	request.setAttribute("memberLoginErrMsg","로그인 오류입니다.");
	//로그인페이지로 포워드(요청 전달)
