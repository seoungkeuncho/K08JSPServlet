...
//page 영역과 request영역에 각각 속성을 저장한다.
...
<head>
<meta charset="UTF-8">
<title>지시어와 액센 태그 동작 방식 비교</title>
</head>
<body>
	<h2>지시어와 액션 태그 동작 방식 비교</h2>
	<!-- 
	지시어의 경우 페이지의 경로를 문자열의 형태로만 기술 할 수 있다.
	표현식을 통해 변수를 사용할수 없다.
	 -->
	<%--정삭적으로 문서가 포함됨 --%>
	<h3>지시어로 페이지 포함하기</h3>
	...
	컴파일을 진행한다. 따라서 하나의 페이지라 생각하면된다.
	그러므로 newVar1은 정상적으로 출력된다.
	--%>
	<p>외부파일에 선언한 변수 : <%=newVar1%></p>
	
	<%--액션태그의 경우 문자열과 표현식 둘다 사용할수있다. --%>
	...
	액션태그
		...
		-하지만 forward처럼 하나의 요청을 공유하므로 reqeust영역은 공유된다.
		-HTML만 있는 경우 주로 사용한다.
	 -->
</body>
</html>