...
<%-- 	<c:set var="id" value="${param.user }" />
	...
	<c:if test="${id eq 'Kosmo' }" var="result">
	</c:if>
	<c:if test="${pass eq '1234' }" var="result">
			kosmo님 하이룽~<br>
	</c:if> --%>
	
	<!-- 최최 실행시에는 파라미터가 없으므로 아무내용도 출력하지 않는다. -->
	<c:if test="${not empty param.user }" >
		전송된 아이디 :${param.user}
		<br>
		전송된 패스워드 :${param.pass}
		<br>
		<!-- 아이디 ,패스워드를 입력햇을때만 일치 여부를 판단한 후 , result에 결과를
		저장한다. 두번째 if문에서 not을 이용해 else구문을 만들면된다. -->
