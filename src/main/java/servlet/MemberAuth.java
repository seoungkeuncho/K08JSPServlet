...
public class MemberAuth extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		/*
		 서블릿 내에서 application내장객체를 얻어온다. 모델 2 방식에서는 서블릿이
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//DAO의 회원 인증을 위한 메서드를 호출한다.
		/*
		 아이디 ,패스워드를 통해 회원인증을 진행한 후 일치하는 정보가 있다면
		 회원 레코드를 DTO에 저장한 후 반환한다.만약 정보가 일치하지 않는다면
		...
		//회원이름을 통해 로그인 성공 여부를 판단한다.
         ...
         if(memberName != null ) {
			...
			//처리한 메세지를 reqeust영역에 저장한 후 JSP로 포워드한다.
